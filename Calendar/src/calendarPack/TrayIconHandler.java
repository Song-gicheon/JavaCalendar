...
public final class TrayIconHandler {
	...
	public static void removeMenuItem(String menu, String label) {
			...
			if (getPopupMenu().getItem(i) instanceof Menu) {
				...
				if (menuitem.getLabel().equals(menu)) {
					for(int k=0; k<menuitem.getItemCount(); k++) {
						if(menuitem.getItem(k).getLabel().equals(label))
							menuitem.remove(k);
	...
	public static void registerTrayIcon(Image image, String toolTip,
		...
		if (SystemTray.isSupported()) {
			...
			try {
				...
				LOGGER.info("I got catch an error during add system tray !"+e);
			}
		} else {
